...
class ChangePasswordUserTest extends TestBase
{
    ...
    public function can_access_to_change_password()
    {
        ...
        $response = $this->get('/cambiar-password/'.$token);

        $response->assertStatus(200)
            ->assertViewIs('pages.change-password')
            ->assertSeeLivewire('pages.change-password', ['token' => $token]);
    }
    ...
    public function check_vars_in_form_change_password()
    {
        ...
        $livewire = Livewire::test(ChangePassword::class, ['token' => $token]);
    ...
    public function check_errors_for_change_password_user($field, $value, $rule)
    {
        ...
        Livewire::actingAs($this->sudo)
            ->test(ChangePassword::class, ['token' => $token])
            ->set($field, $value)
            ->call('save')
    ...
    public function user_can_change_your_password()
    {
        ...
        Livewire::test(ChangePassword::class, ['token' => $token])
